...
public final class Aggregator implements SensorIoAdapter, SolverIoAdapter {
   ...
   * 
   * @param serverConfig
   *          the new configuration values for this aggregator. These are not
   *          used if {@link #init()} has already been invoked.
   */
  ...
  public void init() {
    ...
    this.sensorAcceptor.getFilterChain()
        .addLast(
            "sensor codec",
            new ProtocolCodecFilter(new AggregatorSensorProtocolCodecFactory(
                true)));

    this.solverAcceptor.getFilterChain()
        .addLast(
            "solver codec",
  ...
  public void subscriptionRequestReceived(final IoSession session,
    ...
    log.info("(Solver {}) Responded to subscription request with {}.", solver,
        subscriptionRequestMessage);
  ...
  @Override
  public void exceptionCaught(final IoSession session, final Throwable exception) {
    // No-Op
  }
